...
def evaluate_model(args):
    ...
    for step in range(start_iter, max_iter):
        ...
        if (step % args.vis_freq) == 0:
            if args.type == "vox":
                img_gt = feed_dict['images'][0].detach().cpu().numpy()
                img_gt = img_gt.transpose(0, 1, 2).clip(0,1)
                plt.imsave(f'vis/vox/{step}_{args.type}.png', img_gt)

                predictions_permuted = predictions.permute(0, 1, 4, 2, 3)
                render_voxels(predictions_permuted[0], save_dir='vis/vox/', save_name=f'{step}_{args.type}_pred.gif')
                render_mesh(mesh_gt, save_dir='vis/vox/', save_name=f'{step}_{args.type}_mesh.gif')

            elif args.type == "point":
                img_gt = feed_dict['images'][0].detach().cpu().numpy()
                img_gt = img_gt.transpose(0, 1, 2).clip(0,1)
                # print(img_gt.shape)
                plt.imsave(f'vis/ptc/{step}_{args.type}.png', img_gt)

                render_ptcloud(predictions, save_dir='vis/ptc/', save_name=f'{step}_{args.type}_pred.gif')
                render_mesh(mesh_gt, save_dir='vis/ptc/', save_name=f'{step}_{args.type}_mesh.gif')
            
            elif args.type == "mesh":
                img_gt = feed_dict['images'][0].detach().cpu().numpy()
                img_gt = img_gt.transpose(0, 1, 2).clip(0,1)
                plt.imsave(f'vis/mesh/{step}_{args.type}.png', img_gt)

                render_mesh(predictions, save_dir='vis/mesh/', save_name=f'{step}_{args.type}_pred.gif')
                render_mesh(mesh_gt, save_dir='vis/mesh/', save_name=f'{step}_{args.type}_mesh.gif')
